...
public class RefactoringDataManagement extends DataManagementAdapter {
	...
	private void addPorts() {
		this.inPorts.add(new Port<Transition>(SELECTEDEOBJECT, Transition.class));
		this.inPorts.add(new Port<Transition>("transition", Transition.class));
	}

	@SuppressWarnings("unchecked")
	@Override
	public void preselect(List<EObject> selection) {
		getInPortByName(SELECTEDEOBJECT).setValue((Transition) selection.get(0));
	}
